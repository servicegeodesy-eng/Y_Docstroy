...
function SendToAcknowledgeModal({ cellId, cellName, onClose, onSent }: Props) {
  const { user } = useAuth();
  const { members, loading: loadingMembers } = useEligibleMembers("can_acknowledge");
  ...
  async function handleSend() {
    ...
    // НЕ меняем assigned_to/send_type — ячейка остаётся у отправителя
    await supabase.from("cell_shares").insert({
      cell_id: cellId,
      from_user_id: user.id,
      to_user_id: selectedUserId,
      share_type: "acknowledge",
    ...
    await supabase.from("cell_history").insert({
      cell_id: cellId,
      user_id: user.id,
      action: "sent_to_acknowledge",
      details: { to_user_id: selectedUserId },
    });
    ...
    <div className="ds-overlay p-4" onClick={onClose}>
      ...
      >
        <div className="ds-modal-header">
          <h2 className="ds-modal-title">Отправить на ознакомление</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            {loadingMembers ? (
              ...
              <p className="text-sm" style={{ color: "var(--ds-text-muted)" }}>Нет участников для ознакомления</p>
            ) : (
              <select
...
export default memo(SendToAcknowledgeModal);
