...
interface Props {
  cellId: string;
  cellName: string;
  /** ID пользователя, которому ячейка должна вернуться после проверки (создатель или предыдущий владелец) */
  returnToUserId?: string;
  onClose: () => void;
  onSent: () => void;
}

function SendCellModal({ cellId, cellName, returnToUserId, onClose, onSent }: Props) {
  const { user } = useAuth();
  ...
  async function handleSend() {
    ...
    // Если передан returnToUserId (админ пересылает чужую ячейку), ячейка после проверки вернётся к нему, а не к админу
    ...
    await supabase.from("cell_history").insert({
      ...
      user_id: user.id,
      action: "sent",
      details: { to_user_id: selectedUserId, status: "На проверке" },
    });
    ...
    <div className="ds-overlay p-4" onClick={onClose}>
      ...
      >
        <div className="ds-modal-header">
          <h2 className="ds-modal-title">Отправить на проверку</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            <label className="ds-label">
              Получатель <span style={{ color: "#ef4444" }}>*</span>
            </label>
            {loadingMembers ? (
              <p className="text-sm" style={{ color: "var(--ds-text-faint)" }}>Загрузка...</p>
            ) : members.length === 0 ? (
              <p className="text-sm" style={{ color: "var(--ds-text-muted)" }}>Нет участников с ролью «Проверяющий»</p>
            ) : (
              <select
...
export default memo(SendCellModal);
