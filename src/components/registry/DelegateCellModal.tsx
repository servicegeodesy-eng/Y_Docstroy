...
function DelegateCellModal({ cellId, cellName, originalSenderId, onClose, onDelegated }: Props) {
  const { user } = useAuth();
  ...
  async function handleDelegate() {
    ...
    // 1. Переслать другому проверяющему БЕЗ подписи, сохраняя original_sender_id и send_type
    ...
    // 2. Запись в cell_shares
    await supabase.from("cell_shares").insert({
      cell_id: cellId,
      from_user_id: user.id,
      to_user_id: selectedUserId,
      message: message || "Делегировано другому проверяющему",
    });
    ...
    <div className="ds-overlay p-4" onClick={onClose}>
      ...
      <div className="ds-modal w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <div className="ds-modal-header">
          <h2 className="ds-modal-title">Делегировать</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="ds-alert-info">
            Ячейка будет переслана выбранному проверяющему без подписи. При его действии ячейка вернётся исходному отправителю.
          </div>

          <div>
            <label className="ds-label">
              Проверяющий <span style={{ color: "#ef4444" }}>*</span>
            </label>
            {loadingMembers ? (
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Комментарий к делегированию (необязательно)"
            />
          </div>
...
export default memo(DelegateCellModal);
