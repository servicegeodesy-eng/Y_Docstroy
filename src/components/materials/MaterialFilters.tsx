...
export type MaterialFilterKey = "building" | "workType" | "floor" | "construction";

const FILTER_LABELS: Record<MaterialFilterKey, string> = {
...
interface Props {
  filters: Record<MaterialFilterKey, Set<string>>;
  setFilters: React.Dispatch<React.SetStateAction<Record<MaterialFilterKey, Set<string>>>>;
  filterOptions: Record<MaterialFilterKey, string[]>;
  openFilter: MaterialFilterKey | null;
  setOpenFilter: (f: MaterialFilterKey | null) => void;
  ...
  clearFilters: () => void;
  toggleFilterValue: (key: MaterialFilterKey, value: string) => void;
  allOrders?: boolean;
  setAllOrders?: (v: boolean) => void;
...
export default function MaterialFilters({
  ...
  allOrders, setAllOrders, showAllToggle,
}: Props) {
  ...
  return (
    <div className="ds-card p-3 mb-4 space-y-2">
      <div className="flex items-center gap-2 flex-wrap">
        {showAllToggle && setAllOrders && (
          <>
            <button
              onClick={() => setAllOrders(!allOrders)}
        ...
        {(Object.keys(FILTER_LABELS) as MaterialFilterKey[]).map((key) => {
      ...
      {showSearch && (
        <div className="relative">
          ...
          <input ref={searchInputRef} type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Поиск по номеру заказа, примечанию..."
